...
public class UpnpScanService extends Service {
    ...
    private static final String TAG_TIMECOUNT = "TIMECOUNT";
    ...
    private void doParserAndPlay(Intent intent){
        if (intent.getExtras() != null) {
            ...
            mHandler.post(new Runnable() {     
                  @Override     
                  public void run() { 
                      showToast(getResources().getString(R.string.youtube_url_parser), Toast.LENGTH_SHORT).show();
                  }     
              }); 
            ...
            (new Thread(){
                public void run(){                           
                    try {
                        ...
                        Log.i(TAG_TIMECOUNT, "### TIMECOUNT => Find resource url :" + (parserTime - startTime)/1000.00);
                    } catch (IOException e) {
                    ...
                    if (streamUrl == null) {
                        mHandler.post(new Runnable() {     
                            @Override     
                            public void run() { 
                                showDialog(getResources().getString(R.string.error_title), R.drawable.warning, "1:"+streamError).show();
                            }     
                        }); 
                    }else{
                        mHandler.post(new Runnable() {     
                            @Override     
                            public void run() {     
                                showToast(getResources().getString(R.string.scan), Toast.LENGTH_SHORT).show();
                            }     
                        }); 
                        ...
                        if(size <=0){
                            ...
                            mHandler.post(new Runnable() {     
                                  @Override     
                                  public void run() {     
                                      showToast(getResources().getString(R.string.scan), Toast.LENGTH_LONG).show();
                                  }     
                              }); 
                        }
                    }
    ...
    protected void sendPlay(final int instanceId,  final org.fourthline.cling.model.meta.Service avTransportService) {
        upnpService.getControlPoint().execute( new Play(new UnsignedIntegerFourBytes(instanceId), avTransportService) {
                ...
                public void success(ActionInvocation invocation) {
                    mHandler.post(new Runnable() {     
                          ...
                          public void run() {  
                              ...
                              Log.i(TAG_TIMECOUNT, "### TIMECOUNT => Receive play time: " + (playTime - initTime)/1000.00);
                              showToast(getResources().getString(R.string.send_success), Toast.LENGTH_SHORT).show();
                          }     
                      }); 
                }
                ...
                public void failure(ActionInvocation invocation, UpnpResponse operation, String defaultMsg) {
                    showDialog(getResources().getString(R.string.error_title), R.drawable.warning, "2:"+defaultMsg).show();
                }
            });
    }

    protected boolean sendToMediaRenderer(final int instanceId, DeviceDisplay device, String uri) {
        ...
        Log.i(TAG, "sendToMediaRenderer(), device="+device+", url="+uri+",avTransportService="+avTransportService);        
        if(avTransportService==null){          
            final String errorMessage = "["+device.toString()+"] "+getResources().getString(R.string.not_found_av_transport);                      
            mHandler.post(new Runnable() {     
                @Override     
                public void run() {     
                    showDialog(getResources().getString(R.string.error_title), R.drawable.warning, "3:"+errorMessage ).show();
                }     
            }); 
            return false;
        }
        upnpService.getControlPoint().execute(
            new SetAVTransportURI(new UnsignedIntegerFourBytes(instanceId), avTransportService, uri) {
                ...
                public void success(ActionInvocation invocation) {
                    ...
                    Log.i(TAG_TIMECOUNT, "### TIMECOUNT => Receive init time :" + (initTime - parserTime)/1000.00);
                ...
                public void failure(ActionInvocation arg0, UpnpResponse arg1, String errormsg) {
                    ...
                    mHandler.post(new Runnable() {     
                        @Override     
                        public void run() {     
                            showDialog(getResources().getString(R.string.error_title), R.drawable.warning, "4:"+errorMessage).show();
                        }     
                    }); 
...
//                Log.i(TAG, "-------------------------------------------");
    ...
    private void initUpnpScanService(){
        ...
        mHandler.post(new Runnable() {     
              @Override     
              public void run() { 
                  showToast(getResources().getString(R.string.start_service), Toast.LENGTH_SHORT).show();
              }     
          }); 
    }
